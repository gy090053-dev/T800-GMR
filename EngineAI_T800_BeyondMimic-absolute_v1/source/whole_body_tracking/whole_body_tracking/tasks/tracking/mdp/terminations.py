...
from whole_body_tracking.tasks.tracking.mdp.dead_log import log_ee_body_pos_dead
...
def bad_motion_body_pos_z_only(
    ...
    triggered = torch.any(error > threshold, dim=-1)

    if log_dead and torch.any(triggered):
        log_ee_body_pos_dead(env, command_name, triggered.nonzero(as_tuple=False).flatten())
