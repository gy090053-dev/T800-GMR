...
parser.add_argument("--registry_name", type=str, default=None, help="Wandb artifact name for motion.npz.")
...
def _terminal_hold_episode_length_s(motion_file: str, duration_scale: float = 1.5, minimum_s: float = 10.0) -> float:
    motion = np.load(motion_file)
    fps = float(np.asarray(motion["fps"]).reshape(-1)[0])
    frame_count = int(motion["joint_pos"].shape[0])
    return max(minimum_s, duration_scale * frame_count / fps)
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    if args_cli.motion_file is not None:
        ...
        print(f"[INFO] Using motion artifact: {registry_name}")

    if args_cli.task == "Tracking-Flat-T800-Wo-State-Estimation-Terminal-Hold-v0":
        env_cfg.episode_length_s = _terminal_hold_episode_length_s(env_cfg.commands.motion.motion_file)
        print(f"[INFO] Terminal hold episode length: {env_cfg.episode_length_s:.2f}s")
    ...
    open(os.path.join(log_dir, "dead_log.txt"), "w", encoding="utf-8").close()
